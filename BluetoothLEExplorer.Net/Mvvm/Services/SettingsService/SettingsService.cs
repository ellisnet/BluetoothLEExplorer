//Code from here: https://github.com/Windows-XAML/Template10/blob/version_1.1.12_vs2017/Template10%20(Library)/Services/SettingsService/SettingsService.cs
...
namespace BluetoothLEExplorer.Mvvm.Services.SettingsService
{
    ...
    public class SettingsService : ISettingsService
    {
        ...
        {
            IApplicationDataContainer rootContainer;
            switch (strategy)
            {
                case SettingsStrategies.Local:
                    rootContainer = Application.Current.GetLocalSettingsContainer(); //Application.Current.LocalSettings;
                    break;
                case SettingsStrategies.Roam:
                    rootContainer = Application.Current.GetRoamingSettingsContainer(); //ApplicationData.Current.RoamingSettings;
                    break;
                default:
                    throw new ArgumentException($"Unsupported Settings Strategy: {strategy}", nameof(strategy));
            }

            IApplicationDataContainer targetContainer = rootContainer;
        ...
        protected IApplicationDataContainer Container { get; private set; }
        public IPropertySet Values { get; private set; }

        public IPropertyMapping Converters { get; set; } = new JsonMapping();

        private SettingsService(IApplicationDataContainer container)
        {
            Container = container;
